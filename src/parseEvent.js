export function parseEvent(draftItem, activeDate) {
  ...
  if (draftItem) {
    ...
    date = draftItem.match(/(\d\d\/\d\d)|(\d\d\.\d\d)/i)
      ? draftItem.match(/(\d\d\/\d\d)|(\d\d\.\d\d)/i)[0]
      : "";
    if (date === "") {
    ...
    time = draftItem.match(/(\d\d-\d\d)|(\d\d:\d\d)/i)
      ? draftItem.match(/(\d\d-\d\d)|(\d\d:\d\d)/i)[0]
      : "";
    if (time !== "") {
    ...
    if (
      draftItem.indexOf('"') > -1 &&
      draftItem.lastIndexOf('"') !== draftItem.indexOf('"')
    ) {
      eventItem.header = draftItem.slice(
        draftItem.indexOf('"') + 1,
        draftItem.lastIndexOf('"')
      );
    } else if (
      draftItem.indexOf("'") > -1 &&
      draftItem.lastIndexOf("'") !== draftItem.indexOf("'")
    ...
    if (
      draftItem.indexOf("- ") === -1 ||
      draftItem.indexOf("- ") === draftItem.lastIndexOf("- ")
    ) {
      //inputString without dash+spaces or with one = description only
      let trimText = draftItem;
      if (date > "") {
        trimText = trimText.slice(date.length + 1);
      } // date trim
      if (eventItem.time > "") {
        trimText = trimText.slice(eventItem.time.length + 1);
      } // time trim
      ...
      eventItem.description = eventItem.who.slice(
        eventItem.who.indexOf("- ") + 2
      );
